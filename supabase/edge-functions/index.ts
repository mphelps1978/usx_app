...
import { verifyJWT, generateJWT } from "./jwt.ts";
...
async function handleRequest(request: Request): Promise<Response> {
	...
	try {
		...
		if (path === "/api/loads" && method === "POST") {
			...
			const loadData = (await request.json()) as any;
		...
		if (path === "/api/fuel-stops" && method === "POST") {
			...
			const fuelStopData = (await request.json()) as any;
...
// Password hashing function (basic implementation for Deno)
